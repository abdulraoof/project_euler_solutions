...
for (var i = 0; i < MAX_ROWS; i++) {
    for (var j = 0; j < MAX_COLUMS; j++) {
    	
        if ((j+3) < MAX_COLUMS-1) {
            if ((a[i][j] * a[i][j + 1] * a[i][j + 2] * a[i][j + 3]) > result) {
                result = a[i][j] * a[i][j + 1] * a[i][j + 2] * a[i][j + 3];
            }
        }

        if((i+3) < MAX_ROWS-1 && (j+3) < MAX_COLUMS-1){
			if ((a[i][i] * a[i + 1][j + 1] * a[i + 2][j + 2] * a[i + 3][j + 3]) > result) {
				result = a[i][i] * a[i + 1][j + 1] * a[i + 2][j + 2] * a[i + 3][j + 3];
			}
        }

        if( (i+3) < MAX_ROWS-1){
            if ((a[i][j] * a[i + 1][j] * a[i + 2][j] * a[i + 3][j]) > result) {
                result = a[i][j] * a[i + 1][j] * a[i + 2][j] * a[i + 3][j];
            }
        }

        if( (i+3) < MAX_ROWS-1 && (j-3) > 0){
